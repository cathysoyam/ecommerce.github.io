...
alert(hello);
	...
	var add=document.frm_Records.tb_add.value;
	...
	if(add=="address") {
		...
		if(checkInvalidChar(add)==false && checkInvalidChar(code)==false)
			txt2="Complete Address:"+add+" Zip Code:"+code;
...
function restoreValue(obj) {
	...
	switch(obj) {
		...
		case 4: if(document.frm_Records.tb_code.value!="") {
					oValue=document.frm_Records.tb_code.value;
